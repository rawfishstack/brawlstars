    ...
    protected override void OnAction(ref PlayerDrawSet drawInfo) {
        ...
        if (Player.itemAnimation < 9) {
            for (var i = 0; i < 5; i++) {
                ...
                drawData = Player.BuildPlayerCenterDrawData(Behavior.RequestTexture("Attack_Track"));
...
public class Attack : BrawlerProjectile<Behavior> {
    ...
    public override void SetDefaults() {
        ...
        Projectile.damage = 2800;
        Projectile.light = 5f;
        Projectile.alpha = 255;
    }

    public override void OnDepended() {
        _targetScope = Behavior.AttackAction.TargetScope;
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
        ...
        if (!target.immortal) {
            Behavior.AddChargeValue(35);
        }

        SoundEngine.PlaySound(Behavior.RequestSound("Attack_Hit"));
    }
